...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_purchase_request') {
        ...
        if ($neededDate === null) {
            procurement_set_flash('error', 'Needed date is required.');
            procurement_redirect();
        }

        if ($siteLocation === null) {
            procurement_set_flash('error', 'Site location is required.');
            procurement_redirect();
        }

        if (mb_strlen($itemDescription) < 2) {
            procurement_set_flash('error', 'Item description is too short.');
            procurement_redirect();
        }

        if (mb_strlen($unit) < 2) {
            procurement_set_flash('error', 'Unit is too short.');
            procurement_redirect();
        }
        ...
        $requesterCheck = $conn->prepare("SELECT role, status FROM users WHERE id = ? LIMIT 1");
        $requesterRow = null;
        if ($requesterCheck) {
            $requesterCheck->bind_param('i', $requestedBy);
            $requesterCheck->execute();
            $requesterResult = $requesterCheck->get_result();
            $requesterRow = $requesterResult ? $requesterResult->fetch_assoc() : null;
        }
        if (!$requesterRow || !in_array((string)($requesterRow['role'] ?? ''), ['foreman', 'engineer'], true) || (string)($requesterRow['status'] ?? '') !== 'active') {
            procurement_set_flash('error', 'Requester must be an active engineer or foreman.');
            procurement_redirect();
        }
            ...
            <section class="form-panel">
                ...
                <?php else: ?>
                    <form method="POST">
                        ...
                        <div class="form-grid">
                            <div class="input-group">
                                <label for="project_id">Project <span class="required-indicator" aria-hidden="true">*</span></label>
                            ...
                            <div class="input-group">
                                <label for="requested_by">Requested By <span class="required-indicator" aria-hidden="true">*</span></label>
                                ...
                                <small class="field-help">Foreman ang default gumawa ng request. Engineer puwede lang kung needed.</small>
                            </div>
                            <div class="input-group">
                            ...
                            <div class="input-group">
                                <label for="needed_date">Needed Date <span class="required-indicator" aria-hidden="true">*</span></label>
                                <input type="date" id="needed_date" name="needed_date" required>
                            </div>
                            <div class="input-group">
                                <label for="site_location">Site Location <span class="required-indicator" aria-hidden="true">*</span></label>
                                <input type="text" id="site_location" name="site_location" placeholder="Area, floor, or work zone" required>
                            </div>
                            <div class="input-group">
                                <label for="item_description">Item <span class="required-indicator" aria-hidden="true">*</span></label>
                            ...
                            <div class="input-group">
                                <label for="unit">Unit <span class="required-indicator" aria-hidden="true">*</span></label>
                                <input type="text" id="unit" name="unit" placeholder="bags, pcs, rolls" required>
                            </div>
                            <div class="input-group">
                                <label for="quantity_requested">Quantity <span class="required-indicator" aria-hidden="true">*</span></label>
